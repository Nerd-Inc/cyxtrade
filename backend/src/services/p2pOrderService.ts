...
import { releaseEscrow, refundEscrow } from './walletService';
...
export async function createOrder(userId: string, data: CreateOrderDTO): Promise<P2POrder> {
  return transaction(async (client) => {
    ...
    // Lock seller's wallet row atomically within this transaction
    const walletResult = await client.query(
      `SELECT * FROM user_wallets WHERE user_id = $1 AND asset = $2 FOR UPDATE`,
      [sellerId, escrowAsset]
    );

    const wallet = walletResult.rows[0];
    const currentAvailable = wallet ? Number(wallet.available_balance) : 0;

    if (currentAvailable < escrowAmount) {
      throw new AppError(
        ErrorCode.VALIDATION_ERROR,
        `Insufficient ${escrowAsset} balance. Available: ${currentAvailable}, Required: ${escrowAmount}`
      );
    }
    ...
    await client.query(
      ...
      [sellerId, escrowAsset, escrowAmount]
    );
    ...
    // Create wallet transaction record for escrow lock
    const escrowTxResult = await client.query(
      ...
      ) VALUES ($1, $2, 'escrow_lock', $3, 0, $4, $5, 'completed', 'p2p_order', $6, $7)
      RETURNING *`,
      [
        sellerId,
        escrowAsset,
        escrowAmount,
        currentAvailable,
        currentAvailable - escrowAmount,
        order.id,
        `Escrow lock for order ${order.id}`,
      ]
    );

    const escrowTx = escrowTxResult.rows[0];
